...
from src.primitive_db.decorators import (
    create_cache_manager, handle_database_errors,
    measure_execution_time, require_confirmation
)
from src.primitive_db.utils import load_table_data, save_table_data

SUPPORTED_TYPES = {'int', 'str', 'bool'}

query_cacher = create_cache_manager()
...
@handle_database_errors
@require_confirmation("удаление таблицы")
...
def insert(metadata, table_name, values):
    ...
    if len(values) != len(data_columns):
        print(f'Ошибка: Ожидается {len(data_columns)} '
...
def update(metadata, table_name, set_clause, where_clause):
    ...
    print(f'Запись с ID={updated_ids[0]} в таблице'
          f' "{table_name}" успешно обновлена.')
    return table_data


@handle_database_errors
@require_confirmation("удаление записей")
def delete(metadata, table_name, where_clause, confirm=True):
    ...
    print(f'Запись с ID={deleted_ids[0]} успешно удалена'
